...
if (array_key_exists('increment_quantity_btn', $_POST)) {
    ...
    if ($already_in_shopping_cart) {
        ...
        throw new Exception('WTF YOU CAN NOT ADD A PRODUCT THAT ISNT ALREADY IN YOUR SHOPPING BAG');
}
...
<div class="container my-5">
    ...
    <div class="d-flex flex-column">
        ...
        <div class="progress" style="height: 10px;">
            ...
            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                 style="width: 30.5%; background-color: #e47911;"></div>
            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                 style="width: 20%; background-color: #e47911;"></div>
            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
        ...
        <div class="d-flex flex-row text-uppercase justify-content-around" style="font-size: 12px;">
            ...
            <p class="font-weight-bold">passer commande</p>
            <p class="text-muted">finaliser payment</p>
        </div>
    </div> <!-- Progress Status ends -->

    <h3>Résumé de la commande </h3>
    <p>En passant votre commande, vous acceptez la politique de confidentialité et les conditions d'utilisation de Ece market.</p>
    ...
    <div class="row">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4" style="font-size: 15px;">
                                    <u>
                                        <p class="my-0 py-0">Adresse de livraison</p>
                                    </u> <br>
                                    ...
                                    <p class="my-0 py-0">Téléphone: <?= $addr['mobile_number'] ?></p>
                                    <p class="my-0 py-0"><?= $addr['country'] ?></p>
                                </div>
                                <!-- Billing Address -->
                                <div class="col-md-4" style="font-size: 15px;">
                                    <u>
                                        <p class=" my-0 py-0">Adresse de facturation</p>
                                    </u> <br>
                                    <p class="font-weight-bold my-0 py-0"><?= $addr['owner_name'] ?></p>
                                    <p class="my-0 py-0"><?= $addr['city'] ?></p>
                                    <p class="my-0 py-0"><?= $addr['address'] ?>, <?= $addr['postal_code'] ?></p>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <img src="data:image;base64,<?= base64_encode($product->get_thumbnail()) ?>" class="img-fluid"
                                             alt="product 04 image">
                                    </div>

                                    <!-- Product Detail Col -->
                                    <div class="col-md-6">
                                        <a href="<?= Navigation::product_page($product->get_id()) ?>"><?= $product->get_name() ?></a>
                                        <p class="text-muted my-0">Vendeur
                                            : <?= $account_controller->get_account_name($product->get_seller_account_id()) ?></p>
                                        <p class="text-uppercase font-weight-bold my-0 text-danger">
                                            <span><?= $product->get_price() ?></span> <i class="fas fa-dollar-sign"
                                            ...
                                            <div class="number-input md-number-input">
                                                <form method="POST">
                                                    ...
                                                    <button onclick="this.parentNode.querySelector('input[type=number]').stepDown()"
                                                            ...
                                                            name="decrement_quantity_btn">-
                                                    </button>
                                                    <input class="text" min="1" name="quantity"
                                                           value="<?= $product->get_quantity() ?>" type="number"
                                                           disabled>
                                                    <button onclick="this.parentNode.querySelector('input[type=number]').stepUp()"
                                                            class="btn btn-info btn-sm" type="submit"
                                                            name="increment_quantity_btn">+
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
        ...
        <div class="col-md-4">
            <div class="card mt-md-0 mt-3">
                <div class="card-body">
                    <form method="POST">
                        <button class="btn btn-sm btn-warning btn-block text-capitalize" name="purchase_shopping_bag">
                            Commander
                        </button>
                    </form>
                    <br>
                    <p class="my-0">Résumé de votre commande</p>
                    <p style="font-size: 15px;" class="my-0">Items:
                        <span class="float-right">
                                <?= $total_price ?> <i class="fas fa-dollar-sign"></i>
                            </span>
                    </p>
                    <p style="font-size: 15px;" class="my-0">Livraison:
                        <span class="float-right">
                    ...
                    <br>
                    <div class="dropdown-divider"></div>
                    <p class="my-0 text-danger font-weight-bold">Prix total:
                        <span class="float-right">
                                <?= $total_price ?> <i class="fas fa-dollar-sign"></i>
                            </span>
                    </p>
                ...
                <div class="card-footer">
                    ...
                    <p>Ece market Prime Delivery a été appliqué aux articles éligibles de votre commande .</p>
                    <a href="<?= Navigation::shopping_cart_page() ?>" class="btn btn-danger btn-sm btn-block">Retourner vers le panier</a>
                </div>
            </div>
