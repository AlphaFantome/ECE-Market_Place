...
<div class="container my-5">
    <h3>Laisser un avis au vendeur</h3>
    <p>Produit : <strong><?= $product->get_name() ?></strong> vendu par <em><?= $seller_name ?></em></p>
    ...
    <form method="POST" class="p-5 bg-light rounded" action="<?= Navigation::add_feedback_handler() ?>">
        <div class="form-group">
            <label for="headline" class="font-weight-bold h5">Ajouter un titre</label>
            <input type="text" placeholder="créer un titre pour votre avis" name="headline" class="form-control"
                   id="headline" required>
            <br>
        </div>
        <div class="form-group">
            <label for="review" class="font-weight-bold h5">Commentaire:</label>
            <textarea class="form-control" id="review" name="comments" rows="5" placeholder="Votre commentaire"
                      required></textarea>
        </div>
        ...
        <br>
        <button type="submit" class="btn btn-warning rounded-pill font-weight-bold">Envoyer l'avis</button>
    </form>
</div>
