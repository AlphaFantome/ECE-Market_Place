...
{
    return ($category_code == 1) ? 'art&déco' : (($category_code == 2) ? 'accessoire-vip' : 'materiel-scolaire');
}

function compute_type_of_sale($type_of_sale_code)
{
    return ($type_of_sale_code == 0 ? 'Meilleure offre' : ($type_of_sale_code == 1)) ? 'Enchère' : 'Achat immédiat';
}
...
<div class="container my-5">
    <div class="row">
        <!-- Product list starts -->
        <div class="col-md-9">
            <h1><em><?= count($products) ?> Résultat(s)</em></h1>
            <h10><em>Catégorie : <?= $_GET['category'] ?></em></h10>
            <br>
            <h10><em>Filtre : <?= $_GET['filter'] == '' ? 'empty' : $_GET['filter'] ?></em></h10>
            <br>
            <br>
            ...
            <?php foreach ($products as $product) : $fdb = $feedback_controller->get_feedback_infos($product['id']); ?>
                <div class="row border-top py-3">
                    ...
                    <div class="col-md-8">
                        ...
                        <p class="text-muted my-0">Catégorie : <?= compute_category($product['category']) ?></p>
                        <p class="text-muted my-0">Type de vente
                            : <?= compute_type_of_sale($product['type_of_sale']) ?> </p>
                        <p class="text-muted my-0"><?= $fdb->get_count_reviews() ?> Commentaire(s)</p>
